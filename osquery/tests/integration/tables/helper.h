...
public:
  explicit SpecificValuesCheck(std::initializer_list<std::string> list) : set_(list) {}
...
protected:
  ...
  static bool validate_row(const Row& row, const ValidatatioMap& validation_map);
  static bool validate_rows(const std::vector<Row>& rows, const ValidatatioMap& validation_map);
